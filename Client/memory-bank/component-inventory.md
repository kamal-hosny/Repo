...
export const Sidebar: React.FC<SidebarProps> = ({ userRole, isOpen, onNavigate }) => {
  ...
  return (
    <div className="student-dashboard">
      ...
      <div className="dashboard-grid">
        ...
        <UpcomingDeadlinesCard assignments={getUpcomingDeadlines(assignments)} />
      </div>
    ...
    <div className="admin-dashboard">
      ...
      <div className="dashboard-grid">
        ...
        <UserStatsCard 
          ...
          totalStudents={students?.length || 0}
          onlineTeachers={teachers?.filter(t => t.onlineStatus === 'ONLINE').length || 0}
        />
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (existingSubmission) {
        submission = await submissionAPI.update(existingSubmission.id, { files, notes });
      } else {
        submission = await submissionAPI.create(assignment.id, { files, notes });
      }
      ...
      <form onSubmit={handleSubmit} className="submission-form-content">
        ...
        {!isBeforeDeadline && (
          <Alert variant="warning">
            ...
            {existingSubmission ? ' You can no longer modify your submission.' : ' Late submissions may not be accepted.'}
          </Alert>
        )}
...
export const AssignmentReviewPanel: React.FC<AssignmentReviewPanelProps> = ({
  ...
  const handleGradeSubmit = () => {
    if (selectedSubmission) {
      onGradeSubmission(selectedSubmission.id, gradeForm.grade, gradeForm.feedback);
    ...
    <div className="review-panel">
      <div className="assignment-header">
        ...
        <div className="assignment-stats">
          ...
          <span>Graded: {submissions.filter(s => s.grade !== null).length}</span>
          <span>Pending: {submissions.filter(s => s.grade === null).length}</span>
        </div>
      </div>
...
export const NotificationItem: React.FC<NotificationItemProps> = ({
  ...
  const getNotificationIcon = (type: string) => {
    switch (type) {
      case 'ASSIGNMENT_CREATED': return <AssignmentIcon />;
      case 'GRADE_POSTED': return <GradeIcon />;
      case 'DUE_DATE_APPROACHING': return <ClockIcon />;
